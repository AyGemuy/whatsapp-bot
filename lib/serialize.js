...
const serialize = (m, conn) => {
  if (m.message) {
    ...
    if (
      ...
      message =
        m.message[m.type]?.header || m.message[m.type]?.message || false;
    ...
    m.mentions =
      (message[m.typeV2] || message[m.type])?.contextInfo?.mentionedJid || [];
    m.expiration =
      (message[m.typeV2] || message[m.type])?.contextInfo?.expiration || 0;
  ...
  if (m.quoted.message) {
    m.quoted.key = {
      remoteJid:
        m.message[m.type]?.contextInfo?.remoteJid || m.from || m.sender,
      fromMe:
        decodeJid(m.message[m.type]?.contextInfo?.participant) ===
        conn.user.jid,
      id: m.message[m.type]?.contextInfo?.stanzaId,
      participant:
        decodeJid(m.message[m.type]?.contextInfo?.participant) || m.sender,
    };
    ...
    if (m.quoted) {
      ...
      if (
        ...
        message =
          m.quoted.message[m.quoted.type]?.header ||
          m.quoted.message[m.quoted.type]?.message ||
          false;
      ...
      m.quoted.mentions =
        (message[m.quoted.typeV2] || message[m.quoted.type])?.contextInfo
          ?.mentionedJid || [];
      ...
      if (isMedia) {
        m.quoted.media =
          message[m.quoted.typeV2] || message[m.quoted.type] || null;
  ...
  m.reply = async (textOrOpts, opts = {}) => {
    let text = typeof textOrOpts === "object" && textOrOpts !== null 
  ? (Buffer.isBuffer(textOrOpts.text) ? textOrOpts.text.toString('base64') : JSON.stringify(textOrOpts.text || null, null, 2)) 
  : typeof textOrOpts === "string" ? textOrOpts : null;
let options = typeof textOrOpts === "object" && textOrOpts !== null ? textOrOpts : opts;
    ...
    if (options?.media) {
      ...
      if (/webp/.test(mime)) mtype = "sticker";
        ...
        mtype = Buffer.byteLength(buffer) >= 104857600 ? "document" : "video";
      ...
      let mediaType = Object.keys(options).find((key) =>
        ["image", "video", "document", "sticker", "audio"].includes(key),
      );
      content = {
        caption: text,
        ...options,
        [mediaType]: (await getFile(options[mediaType])).buffer,
      };
    ...
    return conn.sendMessage(from, content, {
      quoted,
      ...expiration,
      messageId,
    });
