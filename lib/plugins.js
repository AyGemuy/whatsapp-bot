...
export function loadPlugins(opts = {}) {
  try {
    ...
    if (opts.table)
      console.table(cmdFiles.map((value) => ({ plugins: value })));
  } catch (e) {
    console.log(e);
  }
}
...
async function addPlugin(filePath, file) {
  ...
  Object.keys(cmd).forEach((type) => {
    ...
    if (typeof cmd[type]?.start === "function") {
      ...
      const existingIndex = plugins[setType].findIndex(
        (existingCmd) => Object.keys(existingCmd)[0] === file,
      );
...
function delPlugin(file) {
  const type = Object.keys(plugins).find((v) =>
    plugins[v].some((c) => Object.keys(c)[0] === file),
  );

  if (plugins[type]) {
    const indexToRemove = plugins[type].findIndex(
      (existingCmd) => Object.keys(existingCmd)[0] === file,
    );
