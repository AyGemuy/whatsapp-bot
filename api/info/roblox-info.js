...
async function getUserInfo(margs, type) {
  try {
    ...
    const userResponse = await fetch("https://users.roblox.com/v1/users/" + id),
      ...
      followersResponse = await fetch(
        "https://friends.roblox.com/v1/users/" + id + "/followers/count",
      ),
      followingsResponse = await fetch(
        "https://friends.roblox.com/v1/users/" + id + "/followings/count",
      ),
      friendsResponse = await fetch(
        "https://friends.roblox.com/v1/users/" + id + "/friends/count",
      ),
      robloxBadgesResponse = await fetch(
        "https://accountinformation.roblox.com/v1/users/" +
          id +
          "/roblox-badges",
      ),
      ...
      roproInfoResponse = await fetch(
        "https://api.ropro.io/getUserInfoTest.php?userid=" + id,
      ),
      roproInfoData = await roproInfoResponse.json(),
      usernameHistoryResponse = await fetch(
