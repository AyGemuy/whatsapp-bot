...
const handler = async (m, conn) => {
  try {
    ...
    const prefixList =
      setting && setting.prefix.length ? setting.prefix : defaultPrefix;
    ...
    const user = isGroup
      ? participants.find((u) => decodeJid(u.id) === m.sender)
      : {};
    const bot = isGroup
      ? participants.find((b) => decodeJid(b.id) === conn.user.jid)
      : {};
      ...
      if (!isBaileys && !isBroadcast) {
        const stickerCommand =
          m.type === "stickerMessage"
            ? db.stickers.get(
                Buffer.from(m.message[m.type].fileSha256).toString("base64"),
              )?.command
            : "";
        ...
        const commands = plugins.commands
          ...
          .filter((commandObj) =>
            commandObj.command.some(
              (cmd) =>
                cmd.toLowerCase() === stickerCommand ||
        ...
        if (commands.length > 0) {
          ...
          for (const cmd of commands) {
            const setting = {
              ...
              isBotAdmin: false,
              usePrefix:
                cmd.setting?.usePrefix !== undefined
                  ? cmd.setting.usePrefix
                  : true,
              ...cmd.setting,
            };
